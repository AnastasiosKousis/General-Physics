...
for t in range(time_steps):
    ...
    if np.mean(T_new[0, :, :]) > 523:
        print("layer gone. Step:", t,". Mean:", np.mean(T_new[0, :, :]), ", Max:", np.max(T_new[0, :, :]))
        T_new = T_new[1:, :, :]
        T_max_arr = T_max_arr[1:,:,:]
        nx -= 1
        total_removed += 1
    if np.mean(T_new[-1, :, :]) > 523:
        print("layer gone. Step:", t,". Mean:", np.mean(T_new[-1, :, :]), ", Max:", np.max(T_new[-1, :, :]))
        T_new = T_new[:-1, :, :]
        T_max_arr = T_max_arr[:-1,:,:]
        nx -= 1
        total_removed += 1
