...
def sutton_groves(v, h):
    q_dot = abs(1.83e-4 * ((air_density(h) / R_t) ** 0.5) * (v ** 3))  # Heat flux (W/m²)
    return q_dot

def heat(T, v, h):
    q = k*(ambient_temp(h)*(1+(v/(np.sqrt(gamma*R*ambient_temp(h)/M)))**2*(gamma-1)/2)-T)
    return q
...
q_dot_0 = heat(T[0,0],velocities[0], heights[0]) * dx / k
T[0, :] += q_dot_0  # Left
T[-1, :] += q_dot_0  # Right
T[:, 0] += q_dot_0  # Bottom
T[:, -1] += q_dot_0  # Top
...
for t in range(time_steps):
    ...
    q_dot_t = heat(T_new[0,0],velocities[t], heights[t]) * dx / k 
